...
const EventCard: FC<EventCardProps> = observer((props) => {
  ...
  const { id, event_images, name, description, navigate, index, productType, price } = props
      ...
      >
        <View
          style={{
            ...
            height: (windowWidth - spacing.md * 2) / eventRatio,
            position: "relative",
          }}
        >
        ...
        <View style={{ padding: spacing.md }}>
          ...
          <Text weight="light" size="sm" style={{ marginBottom: 20 }}>
        ...
        <View style={{ padding: spacing.md }}>
          {data?.pages.map((page) =>
            page.data.map((val: any, index: number) => {
              const { id, event_images, name, description, price, product_type } = val
              return (
                <EventCard
                  navigate={() => _props.navigation.navigate("EventDetail", { id: Number(id) })}
                  id={id}

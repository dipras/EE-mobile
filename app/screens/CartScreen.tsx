...
export const CartScreen: FC<MainTabScreenProps<"Cart">> = observer(function CartScreen(_props) {
  const {
    CartStore: { cartData, removeCartById },
  } = useStores()
  ...
  const pay = () => {
    _props.navigation.navigate("OrderSummary", {
      id: cartData[selected].id,
      image: cartData[selected].imageUrl,
      price: Number(cartData[selected].price),
      productType: cartData[selected].productType,
      name: cartData[selected].name,
    })
  }
  ...
  const renderItem = ({ item, index }: any) => {
    return (
      <View style={{ flexDirection: "row", justifyContent: "center" }} key={index}>
        <TouchableOpacity
          style={[$cart, index === selected ? $cartActive : []]}
      ...
      {cartData[0] ? (
        <>
          ...
          {cartData[selected] ? (
            ...
            <View>
              <Text size="xl" style={{ textAlign: "center", marginBottom: spacing.md }}>

  ...
  return (
    <TouchableOpacity onPress={() => navigate()} style={item} key={index}>
      ...
      <View style={{ padding: spacing.sm }}>
        <Text weight="bold" numberOfLines={2} style={{ height: spacing.xxl }}>
          {name}
        </Text>
        <Text
          weight="light"
          size="sm"
          numberOfLines={4}
          style={{ marginBottom: 20, height: spacing.xxl * 2 }}
        >
  ...
  return (
    <SafeAreaProvider>
      ...
      >
        ...
        <View style={$banner}>
          <Text size="lg" weight="bold" style={{ color: "#4E5566", padding: 20 }}>
            Let's start learning with expert
          </Text>
          <Image
            source={courseBanner}
            style={{ width: windowWidth, height: windowWidth / courseBannerRatio }}
          />
        </View>
        <View style={courseListStyle}>
          {data?.pages.map((page) =>
            page.data.map((val: any, index: number) => {
              const { id, name, description, images, product_type, price } = val
              return (
                <CourseCard
                  index={index}
                  navigate={() => _props.navigation.navigate("CourseDetail", { id: Number(id) })}
                  name={name}
          ...
          {notLastPage &&
            [1, 2].map((val, ind) => (
              <View style={item} key={ind}>
                <View
                  style={{ height: spacing.xxxl * 3, width: "100%", backgroundColor: "#DDD" }}
                ></View>
                <View style={{ padding: spacing.sm }}>
                  <Text
                    weight="bold"
                    numberOfLines={2}
                    style={{ height: spacing.xxl, backgroundColor: "#DDD" }}
                  ></Text>
                  <Text
                    weight="light"
                    size="sm"
                    numberOfLines={4}
                    style={{ marginBottom: 20, height: spacing.xxl * 2, backgroundColor: "#DDD" }}
                  ></Text>
                  <View style={{ backgroundColor: "#DDD", width: "50%", height: 15 }}></View>
                </View>
              </View>
            ))}
        </View>
